...
class ArticleContext
  ...
  def process_image(element, article_image_url)
    ...
    if image_full_url.end_with?('svg')
      return 'missing.png'
    end
...
File.open(image_convert_files, 'w') do |conversion_file|
  IMAGES_TO_PROCESS.each_pair do |key, value|
    unless value.end_with? 'svg'
...
File.open(File.join(BOOK_DIR, 'index.adoc'), 'w') do |out|
  ...
  out << ":figure-caption!:\n"
  if File.exist? 'logo.png'
    out << ":front-cover-image: image:png.png[width=160,height=49]\n"
  end
  out << "\n"
...
p "Converting"
`KINDLEGEN=/usr/local/bin/kindlegen asciidoctor-epub3 -D output -a ebook-format=kf8 data/book/index.adoc`
